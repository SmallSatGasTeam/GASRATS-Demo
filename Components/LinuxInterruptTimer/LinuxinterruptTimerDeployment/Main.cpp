...
#include <LinuxinterruptTimerDeployment/Top/LinuxinterruptTimerDeploymentTopology.hpp>
...
static void signalHandler(int signum) {
    LinuxinterruptTimerDeployment::stopSimulatedCycle();
}
...
int main(int argc, char* argv[]) {
    ...
    LinuxinterruptTimerDeployment::TopologyState inputs;
    ...
    LinuxinterruptTimerDeployment::setupTopology(inputs);
    LinuxinterruptTimerDeployment::startSimulatedCycle(1000);  // Program loop cycling rate groups at 1Hz
    LinuxinterruptTimerDeployment::teardownTopology(inputs);
    (void)printf("Exiting...\n");
    return 0;
}
